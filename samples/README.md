...
> 💡 **JSON을 직접 쓰지 않아도 됩니다.** 대시보드(`streamlit run dashboard.py`)의
> `➕ 업로드 만들기` 탭에서 영상을 올리고 폼을 채우면 mp4와 json을 같은 이름으로 자동 생성해
> 대기열에 넣어줍니다. 아래 내용은 파일을 직접 만들어 넣고 싶을 때 참고하세요.

## 기본 규칙
- `Upload_Queue` 폴더에 **영상(.mp4/.mov)과 JSON(.json)을 같은 이름으로 한 쌍** 넣습니다.
  - 예: `cooking01.mp4` + `cooking01.json`
- 두 파일이 모두 준비되면 데몬이 자동으로 업로드를 시작합니다.
- 확장자와 파일명의 **대소문자는 구분하지 않습니다** (`Clip.MP4` + `clip.json` 도 한 쌍으로 인식).
- 같은 이름을 나중에 다시 넣어도 정상적으로 다시 업로드됩니다.
